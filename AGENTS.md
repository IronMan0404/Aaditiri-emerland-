...
**Staff resident directory** (`/staff/residents`). Security guards and housekeepers can see a read-only directory of approved residents — full name, flat number, phone (tap-to-call), resident type. Nothing else. **Migration:** `supabase/migrations/20260512_staff_resident_directory.sql` (also appended to `supabase/schema.sql`). It registers `public.staff_visible_residents(search_query, page_size, page_offset)` — a `SECURITY DEFINER` function that bypasses RLS on `profiles` but applies an in-body role gate (`auth.uid()` must resolve to a profile with `role IN ('staff','admin')`). The function returns ONLY the four agreed columns; email, avatar, role, push token, audit timestamps, etc. are unreachable. The `/api/staff/residents` route layers a second role check on top so a non-staff caller gets a clean 403. The UI lives at `/staff/residents` with debounced search, "Load more" pagination (50 per page, hard cap 200 per call), and tap-to-call links — no copy-all, no bulk export. The persistent bottom tab strip (`StaffTabs.tsx` mounted in `app/staff/layout.tsx`) is now part of the staff shell. Two tabs: **Home** (their own attendance) and **Residents**. Adding a new staff-facing page = add a new entry to `TABS` in `StaffTabs.tsx`.
