...
- `/api/cron/event-reminders` runs **once daily at 03:30 UTC** (Vercel Hobby plans don't allow more frequent crons; see `vercel.json`). The endpoint is gated by `CRON_SECRET` and does four idempotent passes: event reminders (24-48h ahead), clubhouse pass expiry sweep, clubhouse subscription transitions (active → expiring → expired) with their own `clubhouse_subscription_notices_sent` dedup ledger, and **scheduled reminders** (admin-curated rows in `public.scheduled_reminders`; see `/admin/reminders`). Adding a `Bearer ${CRON_SECRET}` header from any healthcheck is safe to retry.
- **Scheduled reminders — repeating mode** (April 2026, migration `20260513_scheduled_reminders_send_until.sql`). A row may carry an optional `send_until` end date; when set, the daily 09:00 IST sweep fires the same row once each day from `fire_on` through `send_until` (inclusive in IST). Per-day idempotency uses `last_fired_on` (IST date stamp); the `notify()` call uses a per-day dedup refId of `${row.id}:${YYYY-MM-DD}` so the `telegram_notifications_sent` ledger doesn't collapse tomorrow's fire into today's. Single-fire rows (`send_until IS NULL`) keep the original behaviour. The API caps the repeating window at 60 days as a UX nudge — anything longer is almost certainly the wrong tool. `fire-now` is treated as a TERMINAL action: it stamps `last_fired_on` and flips `status='sent'`, regardless of whether the row was repeating, with the UI calling that out at confirm time.
...
**Resident-facing staff directory** (`/dashboard/staff`, added May 2026). Residents (and tenants) can call up the full active staff roster — name, role, photo, on-duty pulse with a live "for X mins" ticker, and tap-to-call phone. **Migration:** `supabase/migrations/20260515_resident_visible_staff.sql` registers `public.resident_visible_staff()`, a SECURITY DEFINER function distinct from `staff_on_duty_now()`. The function is gated to `role IN ('admin','staff') OR (role='user' AND is_approved=true)`, so anon and pending residents get an empty set; bot accounts are also excluded. The projection is name + role + phone + photo + on_duty_since — emails, addresses, hire dates, and audit columns are intentionally NOT exposed. Backed by `/api/staff/directory` (no extra role check needed — the function already enforces it). The page lives at `src/app/dashboard/staff/page.tsx` with role chips (All / Security / Housekeeping), debounced search by name/phone, and on-duty rows pinned to the top with an emerald ring around the avatar. The dashboard quick-links grid grew to 7 tiles (`grid-cols-3 sm:grid-cols-7`) to make room for the new entry. The dashboard `OnDutyCard` (`src/components/dashboard/OnDutyCard.tsx`) still uses the legacy masked-name `staff_on_duty_now()` for the at-a-glance widget — keep them separate, not merged: the widget is a public-screenshot-safe glance card, the directory is a resident-only contact sheet.

**Staff society directory** (`/staff/residents`). Security guards and housekeepers can see a read-only directory of approved residents AND admins — full name, flat number, phone (tap-to-call), resident type for residents / "Admin" badge for office bearers. Nothing else (no email, no avatars, no role audit data). **Migrations:** `supabase/migrations/20260512_staff_resident_directory.sql` introduced the SECURITY DEFINER projection; `supabase/migrations/20260514_staff_directory_include_admins.sql` (May 2026) widened the row filter to `role IN ('user', 'admin')`, returns the new `role` column, and pins admins to the top of the result set so on-duty staff always have an escalation contact. Staff peers (`role='staff'`) are intentionally still excluded. The function — `public.staff_visible_residents(search_query, page_size, page_offset)` — bypasses RLS on `profiles` but applies an in-body role gate (`auth.uid()` must resolve to a profile with `role IN ('staff','admin')`). The `/api/staff/residents` route layers a second role check on top so a non-staff caller gets a clean 403. The UI at `/staff/residents` does debounced search, "Load more" pagination (50 per page, hard cap 200 per call), tap-to-call links, and renders an amber "Admin" pill (with a `ShieldCheck` glyph) on admin rows. The persistent bottom tab strip (`StaffTabs.tsx` mounted in `app/staff/layout.tsx`) is part of the staff shell — two tabs: **Home** (their own attendance) and **Residents** (which is really "Residents + admins" now). Adding a new staff-facing page = add a new entry to `TABS` in `StaffTabs.tsx`.

**Staff analytics for admins** (added May 2026, lives at the bottom of the Attendance tab on `/admin/staff`). Powered by `GET /api/admin/staff/analytics?days=7|30|90`, which rolls up the raw `staff_attendance` rows server-side into: KPIs (active roster split by role, on-duty-now split by role, checked-in-today vs roster size, average daily hours), a today-attendance grid (active staff sorted by first check-in time today, with absent rows surfaced clearly), an hours-on-duty trend line for the window, an "average coverage by hour (IST)" 24-bar histogram derived by walking each shift in 10-minute slices and bucketing by IST hour-of-day, and a top-8 hours-by-staff bar list. Open shifts contribute up to "now" so today's totals don't suddenly drop to zero. Charts reuse `KpiTile`, `LineChart`, `BarRows` and the new `BarHistogram` from `src/components/admin/Charts.tsx` — dependency-free SVG, same precedent as the issues/clubhouse analytics. Range pill selector defaults to 30d. There is no leaves table or shift-start time in V1, so "leaves taken" and "late check-ins" KPIs are intentionally not in this view.
